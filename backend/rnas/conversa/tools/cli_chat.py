...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.image:
            img_path = Path(str(args.image)).expanduser().resolve()
            if not img_path.exists() or not img_path.is_file():
                raise RuntimeError("Arquivo de imagem não encontrado")

            with open(img_path, "rb") as f:
                image_png = f.read()

            res = runtime.reply_with_image(
                user_text,
                image_png=image_png,
                use_ollama=use_ollama,
                model=model,
                image_hint=os.path.basename(str(img_path)),
            )
        else:
