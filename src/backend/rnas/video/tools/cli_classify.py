...
_PARENT = _ROOT.parent.parent  # backend
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))
if str(_PARENT) not in sys.path:
    sys.path.insert(0, str(_PARENT))
# Map rna_de_video to the current module for compatibility
import types
rna_de_video = types.ModuleType('rna_de_video')
rna_de_video.__path__ = [str(_ROOT)]
sys.modules['rna_de_video'] = rna_de_video
from core.classifier import PrototypeClassifier
from core.config import AppConfig, config_from_env, model_dir, thresholds_path
from core.embedding import build_extractor
from core.embedding_cache import get_or_compute_video_embedding
from core.thresholds import Thresholds, load_thresholds
from core.train_modes import build_default_registry
from core.video_frames import probe_video, read_frames_rgb, sample_frame_indices
from core.video_sources import resolve_video_reference_to_file


def _segment_indices(info, *, start_ms: int, end_ms: int, config: AppConfig) -> list[int]:
    if start_ms >= 0 and end_ms > start_ms and info.fps > 1e-6 and info.frame_count > 0 and info.fps < 1e6:
        start_frame = int(min((start_ms / 1000.0) * info.fps, info.frame_count - 1))
        end_frame = int(min((end_ms / 1000.0) * info.fps, info.frame_count))
