            ...
            try:
                for p in videos:
                    ...
                    try:
                        ...
                        if seg_start_ms >= 0 and seg_end_ms > seg_start_ms and info.fps > 1e-6 and info.fps < 1e6:
                        ...
                        if pred.known:
                            ...
                            # Vídeo desconhecido - automaticamente enviar para cluster
                            assign = self._clusterer.assign(emb)
                            conn = connect(dataset_db_path(self.config))
                            init_db(conn)
                            try:
                            ...
                            self._post_log(f"Vídeo analisado - Grupo: {assign.cluster_id} (sim={assign.similarity:.2f}). Vídeo processado com {len(frames)} frames.")
                            self._refresh_clusters()
