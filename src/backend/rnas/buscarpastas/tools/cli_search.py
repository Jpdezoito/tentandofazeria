...
from core.limited_search import limited_search, LimitedSearchConfig  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Layer 3: limited on-demand search without full index
        lim = LimitedSearchConfig(
            max_depth=config.limited_max_depth,
            max_entries=config.limited_max_entries,
            timeout_seconds=config.limited_timeout_seconds,
        )
        limited_hits = limited_search(config, q, qn, cancel, log=None, limits=lim)
        emit_ok({"results": [_result_to_dict(r) for r in limited_hits]})
        return 0
