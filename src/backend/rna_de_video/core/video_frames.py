...
def probe_video(path: Path) -> VideoInfo:
    ...
    if cv2 is not None:
        ...
        try:
            ...
            fps = min(float(cap.get(cv2.CAP_PROP_FPS) or 0.0), 1e6)  # Prevent overflow, max 1M fps
            frame_count = int(min(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0, 1e9))  # Prevent overflow
    ...
    try:
        ...
        fps = min(float(meta.get("fps") or 0.0), 1e6)  # Prevent overflow, max 1M fps
        duration_s = min(float(meta.get("duration") or 0.0), 1e6)  # Prevent overflow, max 1M seconds (~11.5 days)
        frame_count = int(min(meta.get("nframes") or 0, 1e9))  # Prevent overflow
        if frame_count <= 0 and duration_s > 0 and fps > 1e-6:
            frame_count = int(min(duration_s * fps, 1e9))  # Prevent overflow
